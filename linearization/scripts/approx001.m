...
x1=0.05;
appr1=double(subs(L,x,x1));
exct1=(0.95)^(1/3);

% (1.1)^(1/3) = (1-(-0.1))^(!/3); x=-0.1
x2=-0.1;
appr2=double(subs(L,x,x2));
exct2=(1.1)^(1/3);
...
ffn=@(t) arrayfun(@(v) real((1-v)^(1/3)),t);
lfn=@(t) 1-t/3;
...
%plot(xp,ffn(xp),'-','Color',[0 0 0],'LineWidth',1.7,'DisplayName','f(x) = (1-x)^{1/3}');
%hold on;
%plot(xp,lfn(xp),'--','Color',[.45 .45 .45],'LineWidth',1.8,'DisplayName','L(x) = 1-x/3');
...
%plot(xp2,ffn(xp2),'-','Color',[0 0 0],'LineWidth',2,'DisplayName','f(x)');
%hold on;
...
%xlabel('x');
%ylabel('y');
%title('Scaled View of Approximation for: 0.95^{1/3} at x = 0.05');
%legend('Location','northeast');
%grid on;
%hold off;
...
%plot(xp3,ffn(xp3),'-','Color',[0 0 0],'LineWidth',1.8,'DisplayName', 'f(x)');
%hold on;
